...
export class Link {
    ...
    public async sendRequest(
        ...
        try {
            ...
            // app can get broadcast
            if (request.data.req.toJSON()[0] !== 'identity') {
                request.data.flags.broadcast = !broadcast
            }
    ...
    public async login(
        identifier: NameType,
        dapp?: [title: string, icon: string, url: string]
    ): Promise<LoginResult> {
    ...
    getDappInfo(): [title: string, icon: string, url: string] {
        const title = document.title
        const iconDom = document.querySelector<HTMLLinkElement>('link[rel~="icon"]')
        const icon = iconDom ? iconDom.href : ''
